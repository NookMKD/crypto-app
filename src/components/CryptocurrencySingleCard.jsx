...
export default function CurrCard({ coinID, coinData }) {
    const data = coinData;
    ...
    function addToWallet() {
        ...
        if (localStorage.getItem('MyCoins')) {
            if (!myCoins.some(e => e.id == data[coinID].id)) {
    ...
    return (
        <Card sx={{ width: '100%', mb: 2 }} margin={'auto'} justifyContent='center'>
            <CardHeader
                ...
                avatar={
                    < Avatar sx={{ backgroundRepeat: 'no-repeat', }} aria-label='recipe' src={data[coinID].image} />
                }
            >
            </CardHeader>
